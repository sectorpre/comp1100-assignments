  ...
  void . liftIO . forkIO $
    ...
    reflexOf $ mdo
      ...
      let
        ...
        eUpdate =
          let
            -- Apply a series of CodeWorld events to an initial model,
            -- according to the UI's update function. Return the
            -- updated model and any generated responses.
            applyUpdates
              :: UIMode mv
              -> st
              -> model
        ...
        -- constructors of `UIResponse`. (`(<$?>) = mapMaybe`, and
        -- `(<&?>) = flip mapMaybe`, keeping with the convention set
        -- by `(&) = flip ($)`.)
        eMove :: Event _ mv
        eMove =
          let
            -- XXX: It's theoretically possible that we get two move
            -- responses in a single frame. We pick the first one, as
            -- presumably no human can click fast enough to actually
            -- input two responses in one frame.
...
getCodeWorldEvent = do
  ...
  -- There's an `instance Semigroup a => Monoid (Event t a)`, so we
  -- have `mconcat :: [Event t [a]] -> Event t [a]` which collects all
  -- firings in a single frame.
  pure $ mconcat
    -- Note: `pure` for the `[]` applicative is the "singleton list"
    -- function `pure :: a -> [a]`.
